"""The three-stage search funnel described in the project plan:
  ...
  1. Wide flight sweep (sweep_flights) — one calendar request per (origin,
     destination airport, month), building a full grid of (destination,
     depart, return, flight price) candidates. Cheap, because one calendar
     request covers a whole month of daily fares at once: "any Thursday in
     2027" costs 12 requests, not 52.
...
from holiday_tracker.providers.base import FlightProvider, StayProvider
...
def estimate_flight_requests(spec: SearchSpec, city_ids: list[str]) -> int:
    """How many flight-calendar requests sweep_flights will make for this
    spec, so a run can report (or guard on) its cost before spending any of
    the free-tier quota on it."""
    cities = load_cities()
    months = departure_months(spec.date_rule)
    airport_count = sum(len(cities[city_id].iata) for city_id in city_ids)
    return len(spec.origins) * airport_count * len(months)


def sweep_flights(
    spec: SearchSpec, city_ids: list[str], flight_provider: FlightProvider
) -> list[Candidate]:
    """Stage 1: build the full (origin, destination, depart, return) grid,
    with a flight price attached to every date pair the calendar happened
    to have a cached fare for."""
    cities = load_cities()
    date_pairs = expand_date_pairs(spec.date_rule)
    months = departure_months(spec.date_rule)
    ...
    for city_id in city_ids:
        ...
        for origin in spec.origins:
            for destination_iata in city.iata:
                calendars = {
                    (year, month): flight_provider.fare_calendar(
                        origin, destination_iata, year, month
                    )
                    for year, month in months
                }
                for depart, return_ in date_pairs:
                    calendar = calendars.get((depart.year, depart.month))
                    if calendar is None:
                        continue
                    price = calendar.prices.get(depart)
                    if price is None:
                        continue  # no cached fare for this specific departure day
                    candidates.append(
                        Candidate(
                            origin=origin,
